...
class MonkAgent(BaseAgent):
    ...
    def action_pattern_match_behavior(
        self, params: Dict[str, Any], wade_core
    ) -> Dict[str, Any]:
        """
        ...
        if recent_commands:
            ...
            recon_count = sum(
                ...
                if any(kw in cmd["data"].lower() for kw in recon_keywords)
            )
            security_count = sum(
                1
                for cmd in recent_commands
                if any(kw in cmd["data"].lower() for kw in security_keywords)
            )
            dev_count = sum(
                1
                for cmd in recent_commands
                if any(kw in cmd["data"].lower() for kw in dev_keywords)
            )
            ...
            elif security_count >= 2 and any(
                kw in command.lower() for kw in security_keywords
            ):
        ...
        if pattern:
            if pattern not in self.state["behavioral_patterns"]:
                ...
                self.state["behavioral_patterns"][pattern]["confidence"] = max(
                    self.state["behavioral_patterns"][pattern]["confidence"], confidence
                )
    ...
    def action_validate_user_logic(
        self, params: Dict[str, Any], wade_core
    ) -> Dict[str, Any]:
        """
        ...
        for term1, term2 in contradictions:
            if term1 in command.lower() and term2 in command.lower():
                is_valid = False
                reason = (
                    f"Command contains contradictory terms: '{term1}' and '{term2}'"
                )
                break

        # Check for incomplete commands
        if (
            command.lower().startswith(("if ", "when ", "while "))
            and " then " not in command.lower()
        ):
            is_valid = False
        ...
        if (
            any(term in command.lower() for term in vague_terms)
            and len(command.split()) < 10
        ):
            is_valid = False
            reason = "Command is too vague or ambiguous"

        return {"status": "success", "data": {"is_valid": is_valid, "reason": reason}}

    def action_analyze_context(
        self, params: Dict[str, Any], wade_core
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "data": {
                ...
                "context_stability": len(context_shifts)
                < 3,  # Context is stable if few shifts
            },
        }
