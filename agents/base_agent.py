...
class BaseAgent(ABC):
    ...
    def execute_action(self, action: str, params: Dict[str, Any], wade_core) -> Dict[str, Any]:
        """
        ...
        if not hasattr(self, f"action_{action}") or not callable(getattr(self, f"action_{action}")):
            return {
                'status': 'error',
    ...
    def get_action_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        return sorted(self.action_history, key=lambda x: x.get('start_time', 0), reverse=True)[:limit]
    ...
    def get_capabilities(self) -> List[str]:
        ...
        """
        return [method[7:] for method in dir(self) if method.startswith('action_') and callable(getattr(self, method))]
