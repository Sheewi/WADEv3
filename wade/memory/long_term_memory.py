...
class LongTermMemory:
    ...
    def __init__(self, storage_type: str = "json", max_size: int = 10000, backup_interval: int = 3600):
        """
    ...
    def _setup_storage(self):
        """Set up storage."""
        # Get storage path
        self.storage_path = os.environ.get('WADE_RUNTIME', os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'WADE_RUNTIME'))
    ...
    def _backup_loop(self):
        ...
        while self.is_running:
            try:
                ...
                if self.is_modified and time.time() - self.last_backup > self.backup_interval:
                    self._save_memory()
    ...
    def retrieve_with_metadata(self, key: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error retrieving value with metadata for key '{key}': {e}")
            return None
