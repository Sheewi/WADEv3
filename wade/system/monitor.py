...
class SystemMonitor:
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            ...
            "custom_metrics": {"enabled": True, "endpoint": "/metrics"},
        }
    ...
    def _collect_process_metrics(self) -> Dict[str, float]:
        ...
        try:
            for process_name, config in self.config["processes"].items():
                ...
                metrics[f"process_{process_name}_memory_mb"] = total_memory / (
                    1024 * 1024
                )
    ...
    def _store_metrics(self, metrics: Dict[str, float]):
        ...
        try:
            ...
            self.metrics_buffer.append({"timestamp": timestamp, "metrics": metrics})
    ...
    def _check_thresholds(self, metrics: Dict[str, float]):
        ...
        try:
            for metric_name, value in metrics.items():
                ...
                if (
                    "critical" in threshold_config
                    and value >= threshold_config["critical"]
                ):
                    alert_level = "critical"
                    threshold_value = threshold_config["critical"]
                elif (
                    "warning" in threshold_config
                    and value >= threshold_config["warning"]
                ):
                    alert_level = "warning"
    ...
    def get_metric_history(self, metric_name: str, hours: int = 24) -> List[Dict]:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                return [
                    {"timestamp": row[0], "value": row[1]} for row in cursor.fetchall()
                ]
    ...
    def get_system_health(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "critical_alerts": len(
                    [a for a in active_alerts if a.level == "critical"]
                ),
                "warning_alerts": len(
                    [a for a in active_alerts if a.level == "warning"]
                ),
