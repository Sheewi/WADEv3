...
class LogRotator:
    ...
    def create_timed_logger(
        self, name: str, log_file: str = None, when: str = "midnight", interval: int = 1
    ) -> logging.Logger:
        """
    ...
    def export_logs(
        self, start_date: datetime, end_date: datetime, log_types: List[str] = None
    ) -> str:
        """
    ...
    def search_logs(
        self, pattern: str, log_files: List[str] = None, max_results: int = 1000
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            results.append({"error": f"Search error: {e}"})
