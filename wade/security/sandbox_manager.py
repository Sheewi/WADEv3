...
class SandboxManager:
    ...
    def create_sandbox(
        self, sandbox_type: str = "default", name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _create_chroot_sandbox(self, sandbox_id: str, name: str) -> Dict[str, Any]:
        ...
        chroot_dir = os.path.abspath(
            os.path.join(
                os.path.dirname(os.path.dirname(__file__)),
                "WADE_RUNTIME",
                "temp",
                f"chroot_{sandbox_id}",
            )
        )
        ...
        try:
            ...
            for binary in ["sh", "ls", "cat", "echo"]:
                binary_path = subprocess.run(
                    ["which", binary], stdout=subprocess.PIPE, text=True
                ).stdout.strip()
                if binary_path:
    ...
    def execute_in_sandbox(
        ...
        if sandbox_id not in self.active_sandboxes:
            return {"status": "error", "message": f"Sandbox not found: {sandbox_id}"}
    ...
    def _execute_in_default_sandbox(
        self, sandbox_info: Dict[str, Any], command: str, timeout: int
    ) -> Dict[str, Any]:
        """
    ...
    def _execute_in_chroot_sandbox(
        self, sandbox_info: Dict[str, Any], command: str, timeout: int
    ) -> Dict[str, Any]:
        """
    ...
    def _destroy_docker_sandbox(self, sandbox_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            subprocess.run(
                ["docker", "stop", sandbox_info["container_name"]],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
            subprocess.run(
                ["docker", "rm", sandbox_info["container_name"]],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
