...
class WADEBootloader:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.animation_speed = self.boot_config.get('display', {}).get('animation_speed', 0.1)
        ...
        try:
            if os.path.exists(self.config_path):
                with open(self.config_path, 'r') as f:
                    config = json.load(f)
                    # Merge with defaults
                    return self._merge_configs(default_config, config.get('bootloader', {}))
...
"""
    ...
    def _show_loading_animation(self):
        ...
        for i in range(10):
            print(f"\r\033[33m{dots[i % len(dots)]} Loading...\033[0m", end="", flush=True)
    ...
    def _execute_stage(self, stage: Dict) -> bool:
        ...
        try:
            ...
            if result:
                if self.display_enabled:
                    print(f"\r\033[32m✓ {stage['name']} completed ({stage_time:.1f}s)\033[0m")
                return True
            else:
                if self.display_enabled:
                    print(f"\r\033[31m✗ {stage['name']} failed ({stage_time:.1f}s)\033[0m")
                return False
    ...
    def _stage_system_check(self) -> bool:
        ...
        try:
            ...
            required_version = tuple(map(int, self.boot_config['dependencies']['python_version'].split('.')))
            
            if python_version < required_version:
                self.errors.append(f"Python {self.boot_config['dependencies']['python_version']} required, found {sys.version}")
                return False
            ...
            if memory_mb < requirements['min_memory_mb']:
                self.errors.append(f"Insufficient memory: {memory_mb:.0f}MB < {requirements['min_memory_mb']}MB")
                return False
            
            # Disk check
            disk_gb = psutil.disk_usage('/').free / (1024 * 1024 * 1024)
            if disk_gb < requirements['min_disk_gb']:
                self.errors.append(f"Insufficient disk space: {disk_gb:.1f}GB < {requirements['min_disk_gb']}GB")
                return False
            
            # CPU check
            cpu_cores = psutil.cpu_count()
            if cpu_cores < requirements['min_cpu_cores']:
                self.errors.append(f"Insufficient CPU cores: {cpu_cores} < {requirements['min_cpu_cores']}")
                return False
    ...
    def _stage_final_checks(self) -> bool:
        ...
        try:
            ...
            critical_services = (self.boot_config['services']['core'] + 
...
def main():
    ...
    parser.add_argument('--safe-mode', '-s', action='store_true', help='Boot in safe mode')
    parser.add_argument('--no-display', '-n', action='store_true', help='Disable visual display')
