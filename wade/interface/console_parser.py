...
class ConsoleParser:
    ...
    def _register_built_in_commands(self):
        """Register built-in commands."""
        self.register_command('help', self._cmd_help, 'Display help information', ['?', 'h'])
        ...
        self.register_command('history', self._cmd_history, 'Show command history', ['hist'])
        self.register_command('status', self._cmd_status, 'Show WADE status', ['stat'])
        self.register_command('version', self._cmd_version, 'Show WADE version', ['ver'])
        self.register_command('reload', self._cmd_reload, 'Reload WADE configuration', ['refresh'])
        self.register_command('agents', self._cmd_agents, 'List available agents', ['agent'])
        self.register_command('tools', self._cmd_tools, 'List available tools', ['tool'])
    
    def register_command(self, command: str, handler: Callable, help_text: str, aliases: Optional[List[str]] = None):
        """
    ...
    def _cmd_help(self, args: str) -> str:
        ...
        for cmd in sorted(self.command_handlers.keys()):
            aliases = [alias for alias, target in self.command_aliases.items() if target == cmd]
    ...
    def _cmd_agents(self, args: str) -> str:
        ...
        for agent_type in sorted(agent_types):
            active_count = sum(1 for agent in active_agents.values() if agent.agent_type == agent_type)
    ...
    def _cmd_tools(self, args: str) -> str:
        ...
        for tool_name, tool_info in sorted(tools.items()):
            tool_lines.append(f"  {tool_name}: {tool_info.get('description', 'No description')}")
