...
class AnimatedWidget:
        ...
        for i in range(glow_radius):
            ...
            if style['shape'] == 'circle':
                ...
                glow_element = self.canvas.create_rectangle(
                    self.x - self.width//2 - i, self.y - self.height//2 - i,
                    self.x + self.width//2 + i, self.y + self.height//2 + i,
                    fill='',
                    outline=color,
                    width=1
    ...
    def _animate(self):
        ...
        self.glow_intensity = (math.sin(self.animation_frame * self.theme.ANIMATION['glow_speed']) + 1) / 2
...
class SystemStatusWidget(AnimatedWidget):
    ...
    def _create_content(self, style: Dict):
        ...
        self.cpu_arc = self.canvas.create_arc(
            self.x - 40, self.y - 40, self.x + 40, self.y + 40,
            start=90, extent=0,
    ...
    def _refresh_content(self):
        ...
        color = self.theme.COLORS['success'] if cpu_percent < 90 else self.theme.COLORS['warning']
...
class NetworkStatusWidget(AnimatedWidget):
    ...
    def _refresh_content(self):
        ...
        color = self.theme.COLORS['success'] if connected else self.theme.COLORS['error']
...
class AgentStatusWidget(AnimatedWidget):
    ...
    def _create_content(self, style: Dict):
        ...
        self.elements.extend([self.agent_count_text, self.agent_label] + self.activity_dots)
    ...
    def _refresh_content(self):
        ...
        for i, dot in enumerate(self.activity_dots):
            color = (self.theme.COLORS['accent_cyan'] if i < active_agents 
...
class SecurityStatusWidget(AnimatedWidget):
    ...
    def _create_content(self, style: Dict):
        ...
        shield_points = [
            ...
            self.x - 20, self.y - 10, # Top left
            self.x - 20, self.y + 10, # Bottom left
            self.x, self.y + 30,      # Bottom
            self.x + 20, self.y + 10, # Bottom right
            self.x + 20, self.y - 10, # Top right
        ]
...
def main():
    ...
    parser.add_argument('--windowed', '-w', action='store_true', help='Run in windowed mode')
    parser.add_argument('--no-particles', action='store_true', help='Disable particle effects')
    parser.add_argument('--no-grid', action='store_true', help='Disable grid background')
