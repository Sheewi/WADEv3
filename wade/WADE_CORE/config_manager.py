...
class ConfigManager:
    ...
    def _get_default_config(self) -> Dict:
        ...
        return {
            ...
            "database": {"type": "sqlite", "name": "/var/lib/wade/wade.db"},
            "config": {
                "hot_reload": True,
        ...
        if os.path.exists(env_config_path):
            try:
                with open(env_config_path, "r") as f:
                    if env_config_path.endswith(".yaml") or env_config_path.endswith(
                        ".yml"
                    ):
    ...
    def _apply_env_variables(self):
        ...
        for env_var, config_path in env_mappings.items():
            if env_var in os.environ:
                ...
                # Type conversion
                if (
                    config_path.endswith(".port")
                    or config_path.endswith(".timeout")
                    or config_path.endswith(".max_concurrent")
                ):
                    value = int(value)
    ...
    def remove_watcher(self, key: str, callback: Callable[[str, Any], None]):
        ...
        """
        self.watchers = [
            (k, c) for k, c in self.watchers if not (k == key and c == callback)
        ]
