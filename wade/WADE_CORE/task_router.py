...
class TaskRouter:
    ...
    def _route_to_agents(self, task: str) -> Optional[str]:
        ...
        for agent_id, agent in active_agents.items():
            try:
                ...
                self.logger.error(
                    f"Error processing task with agent {agent_id}: {str(e)}"
                )
    ...
    def _handle_system_info_task(self, task: str) -> str:
        ...
        if result.get("status") == "success":
            ...
            response += (
                f"Platform: {info.get('platform')} {info.get('platform_release')}\n"
            )
    ...
    def _handle_file_list_task(self, task: str) -> str:
        ...
        if result.get("status") == "success":
            ...
            return (
                f"Sorry, I couldn't list files in {path}. {result.get('message', '')}"
            )
    ...
    def _handle_memory_store_task(self, task: str) -> str:
        ...
        key_match = re.search(
            r'(?i)(?:remember|store|save)\s+(?:that|this|it)?\s*(?:as|with key|with name)?\s*["\']?([^"\']+)["\']?',
            task,
        )
        ...
        value_match = re.search(
            r'(?i)(?:remember|store|save)\s+(?:that|this|it)?\s*["\']?([^"\']+)["\']?',
            task,
        )
    ...
    def _handle_memory_recall_task(self, task: str) -> str:
        ...
        key_match = re.search(
            r'(?i)(?:recall|retrieve|get|what).*(?:about|regarding|concerning|on)\s+["\']?([^"\']+)["\']?',
            task,
        )
    ...
    def _handle_tool_execution_task(self, task: str) -> str:
        ...
        tool_match = re.search(
            r'(?i)(?:run|execute|use)\s+(?:tool|command)?\s*["\']?([^"\']+)["\']?', task
        )
