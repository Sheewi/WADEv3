...
class ErrorHandler:
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            ...
            "notifications": {
                ...
                "webhook": {"enabled": False, "url": None, "timeout": 10},
            },
            "recovery": {
                "enabled": True,
                "max_attempts": 3,
        ...
        try:
            with self.error_lock:
                ...
                if self._is_rate_limited(error_context):
                    self.logger.warning(
                        "Error rate limit exceeded, suppressing notifications"
                    )
    ...
    def _categorize_error(self, error: Exception) -> ErrorCategory:
        ...
        if any(
            keyword in error_type or keyword in error_msg
            for keyword in [
                "permission",
                "auth",
                "login",
            ...
            keyword in error_type or keyword in error_msg
            for keyword in ["network", "connection", "socket", "timeout", "dns", "http"]
        ):
            return ErrorCategory.NETWORK

        elif any(
            keyword in error_type or keyword in error_msg
            for keyword in ["database", "sql", "db", "sqlite", "postgres", "mysql"]
        ):
            ...
            keyword in error_type or keyword in error_msg
            for keyword in ["agent", "task", "execution", "worker"]
        ):
            return ErrorCategory.AGENT

        elif any(
            keyword in error_type or keyword in error_msg
            for keyword in ["memory", "disk", "cpu", "resource", "limit", "quota"]
        ):
    ...
    def _assess_severity(self, error: Exception) -> ErrorSeverity:
        ...
        if any(
            keyword in error_type or keyword in error_msg
            for keyword in ["fatal", "critical", "system", "memory", "corruption"]
        ):
        ...
        # Medium severity errors
            ...
            keyword in error_type or keyword in error_msg
            for keyword in ["warning", "deprecated", "invalid"]
        ):
    ...
    def _log_error(self, error_context: ErrorContext):
        ...
        if error_context.severity in [
            ...
            self.logger.error(
                f"Traceback for {error_context.error_id}:\n{error_context.traceback_info}"
            )
    ...
    def _is_rate_limited(self, error_context: ErrorContext) -> bool:
        ...
        minute_count = sum(
            ...
            if err.category.value == category and err.timestamp.timestamp() > minute_ago
        )

        hour_count = sum(
            1
            for err in self.error_log
            if err.category.value == category and err.timestamp.timestamp() > hour_ago
        )
    ...
    def _send_email_notification(self, error_context: ErrorContext):
        ...
        try:
            ...
            msg["Subject"] = (
                f"WADE {error_context.severity.value.upper()} Error: {error_context.error_id}"
            )
...
"""
    ...
    def _send_webhook_notification(self, error_context: ErrorContext):
        ...
        try:
            ...
            response = requests.post(
                webhook_config["url"], json=payload, timeout=webhook_config["timeout"]
            )
    ...
    def _attempt_recovery(self, error_context: ErrorContext):
        ...
        for attempt in range(max_attempts):
            try:
                self.logger.info(
                    f"Attempting recovery for {error_context.error_id}, attempt {attempt + 1}"
                )
                ...
                if success:
                    ...
                    error_context.recovery_actions.append(
                        f"Recovery successful on attempt {attempt + 1}"
                    )
                    self.logger.info(
                ...
                error_context.recovery_actions.append(
                    f"Recovery attempt {attempt + 1} failed: {str(e)}"
                )
                self.logger.error(
                    f"Recovery attempt {attempt + 1} failed for {error_context.error_id}: {e}"
                )
    ...
    def _check_system_protection(self, error_context: ErrorContext):
        ...
        try:
            ...
            if (
                critical_resource
                and error_context.severity == ErrorSeverity.FATAL
                and protection_config["emergency_shutdown"]
            ):

                self.logger.critical(
                    "Initiating emergency shutdown due to critical resource usage and fatal error"
                )
    ...
    def _update_circuit_breaker(self, error_context: ErrorContext):
        ...
        if (
            ...
            and breaker["failure_count"] >= config["failure_threshold"]
        ):
    ...
    def register_error_callback(
        self, category: ErrorCategory, callback: Callable[[ErrorContext], None]
    ):
    ...
    def register_recovery_strategy(
        self, category: ErrorCategory, strategy: Callable[[ErrorContext], bool]
    ):
    ...
    def get_error_stats(self) -> Dict:
        ...
        with self.error_lock:
            ...
            recent_errors = [
                err for err in self.error_log if err.timestamp.timestamp() > hour_ago
            ]
