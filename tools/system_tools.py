...
class FileSystemTool(BaseTool):
    ...
    def _run(
        self, action: str, path: str, content: str = "", permissions: str = ""
    ) -> str:
        """
...
class ProcessManagerTool(BaseTool):
    ...
    def _start_service(self, service: str) -> str:
        ...
        try:
            result = subprocess.run(
                ["sudo", "systemctl", "start", service], capture_output=True, text=True
            )
    ...
    def _stop_service(self, service: str) -> str:
        ...
        try:
            result = subprocess.run(
                ["sudo", "systemctl", "stop", service], capture_output=True, text=True
            )
            if result.returncode == 0:
...
class NetworkManagerTool(BaseTool):
    ...
    def _run(
        self, action: str, interface: str = "", ip: str = "", options: str = ""
    ) -> str:
        """
...
class ToolManagerTool(BaseTool):
    ...
    def _run(
        self, action: str, tool_name: str = "", code: str = "", language: str = "python"
    ) -> str:
        """
        ...
        try:
            ...
            for filepath in TOOLS_DIR.glob("*"):
                if filepath.is_file():
                    stat = filepath.stat()
                    tools.append(
                        f"{filepath.name:<30} {stat.st_size:>8} bytes  {time.ctime(stat.st_mtime)}"
                    )
