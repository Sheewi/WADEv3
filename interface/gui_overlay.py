...
class GUIOverlay:
            ...
            return jsonify(
                {
                    ...
                    "uptime": time.time()
                    - getattr(self.elite_few.wade_core, "start_time", time.time()),
                }
            )
    ...
    def _setup_socketio_events(self):
        ...
        def handle_command(data):
            ...
            if command:
                ...
                self.socketio.emit(
                    "command_received",
                    {"status": "success", "command": command, "timestamp": time.time()},
                    room=request.sid,
                )
    ...
    def stop(self):
        ...
        if self.is_running and self.socketio:
            ...
            self.socketio.emit(
                "server_shutdown",
                {"message": "Server shutting down", "timestamp": time.time()},
            )
