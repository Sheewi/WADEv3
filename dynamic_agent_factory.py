...
class DynamicAgentFactory:
    ...
    def __init__(self, model_router=None, config_path: str = None):
        ...
        self.config_path = config_path or os.path.expanduser("~/.wade/config/agents.json")
    ...
    def _load_templates(self):
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                logger.warning(f"Agent templates file not found at {self.config_path}, creating default templates")
    ...
    def _save_templates(self):
        ...
        try:
            ...
            logger.info(f"Saved {len(self.agent_templates)} agent templates to {self.config_path}")
        except Exception as e:
    ...
    def add_template(self, name: str, template: Dict[str, Any]) -> bool:
        ...
        if name in self.agent_templates:
            logger.warning(f"Template '{name}' already exists, use update_template to modify it")
            return False
    ...
    def update_template(self, name: str, template: Dict[str, Any]) -> bool:
        ...
        if name not in self.agent_templates:
            logger.warning(f"Template '{name}' does not exist, use add_template to create it")
            return False
    ...
    def create_agent(self, template_name: str, agent_id: str = None, 
                   custom_params: Dict[str, Any] = None) -> Optional[Agent]:
    ...
    async def create_agent_from_description(self, description: str) -> Optional[str]:
        ...
        try:
            ...
            if self.model_router:
                response = await self.model_router.generate(prompt, required_capabilities=["reasoning"])
    ...
    def create_crew(self, agent_ids: List[str], tasks: List[Dict[str, Any]] = None) -> Optional[Crew]:
            ...
            for i, task_params in enumerate(tasks):
                if i < len(agents):
                    task = Task(
                        ...
                        expected_output=task_params.get("expected_output", "Completed task"),
                        agent=agents[i]
                    )
    ...
    async def analyze_task_and_create_agents(self, task_description: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for agent_info in analysis.get("agents", []):
                ...
                if agent_type in self.agent_templates:
                    ...
                    if self.model_router:
                        agent_response = await self.model_router.generate(new_agent_prompt, required_capabilities=["reasoning"])
                        agent_json_str = agent_response.get("response", "")
                    else:
                        # Use the default LLM
                        agent_response = requests.post("http://localhost:11434/api/generate", json={
                            "model": "phind-codellama",
                    ...
                    # Extract and parse the JSON
                    agent_json_match = re.search(r'```json\n(.*?)\n```', agent_json_str, re.DOTALL)
                    if agent_json_match:
                        agent_json_str = agent_json_match.group(1)
                    else:
                        # Try to find JSON without the markdown code block
                        agent_json_match = re.search(r'(\{.*\})', agent_json_str, re.DOTALL)
                        if agent_json_match:
                            agent_json_str = agent_json_match.group(1)
