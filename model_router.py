...
class ModelRouter:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.routing_rules = []
        self.config_path = config_path or os.path.expanduser("~/.wade/config/models.json")
        self._load_config()
    ...
    def add_model(self, name: str, model_type: ModelType, endpoint: str, parameters: Dict[str, Any], 
                 capabilities: List[str] = None, context_window: int = 4096):
    ...
    def add_routing_rule(self, rule_type: str, pattern: str, target_model: str, priority: int = 0):
        """
        Add a routing rule
    ...
    def _match_rule(self, query: str, context_length: int = 0, required_capabilities: List[str] = None):
    ...
    def select_model(self, query: str, context_length: int = 0, required_capabilities: List[str] = None):
        """
    ...
    async def generate(self, query: str, model_name: str = None, context: List[Dict[str, str]] = None, 
        ...
        if not model_name:
            ...
            context_length = sum(len(msg.get("content", "")) for msg in context) + len(query)
        ...
        if model_type == ModelType.OLLAMA.value:
            return await self._generate_ollama(query, model_config, merged_parameters, context)
        elif model_type == ModelType.OPENAI.value:
            return await self._generate_openai(query, model_config, merged_parameters, context)
        elif model_type == ModelType.ANTHROPIC.value:
            return await self._generate_anthropic(query, model_config, merged_parameters, context)
        elif model_type == ModelType.HUGGINGFACE.value:
            return await self._generate_huggingface(query, model_config, merged_parameters, context)
        elif model_type == ModelType.LOCALAI.value:
            return await self._generate_localai(query, model_config, merged_parameters, context)
        elif model_type == ModelType.CUSTOM.value:
            return await self._generate_custom(query, model_config, merged_parameters, context)
        else:
            raise ValueError(f"Unsupported model type: {model_type}")
            
    async def _generate_ollama(self, query: str, model_config: Dict[str, Any], 
                             parameters: Dict[str, Any], context: List[Dict[str, str]]):
    ...
    async def _generate_openai(self, query: str, model_config: Dict[str, Any], 
                             parameters: Dict[str, Any], context: List[Dict[str, str]]):
        """Generate using OpenAI API"""
        import openai
        
        # Set API key from config or environment
        api_key = parameters.get("api_key") or model_config.get("api_key") or os.environ.get("OPENAI_API_KEY")
        if not api_key:
    ...
    async def _generate_anthropic(self, query: str, model_config: Dict[str, Any], 
                                parameters: Dict[str, Any], context: List[Dict[str, str]]):
        """Generate using Anthropic API"""
        import anthropic
        
        # Set API key from config or environment
        api_key = parameters.get("api_key") or model_config.get("api_key") or os.environ.get("ANTHROPIC_API_KEY")
        if not api_key:
    ...
    async def _generate_huggingface(self, query: str, model_config: Dict[str, Any], 
                                  parameters: Dict[str, Any], context: List[Dict[str, str]]):
        """Generate using Hugging Face Inference API"""
        import requests
        
        # Set API key from config or environment
        api_key = parameters.get("api_key") or model_config.get("api_key") or os.environ.get("HF_API_KEY")
        if not api_key:
    ...
    async def _generate_localai(self, query: str, model_config: Dict[str, Any], 
                              parameters: Dict[str, Any], context: List[Dict[str, str]]):
    ...
    async def _generate_custom(self, query: str, model_config: Dict[str, Any], 
                             parameters: Dict[str, Any], context: List[Dict[str, str]]):
        ...
        for key, value_template in payload_format.items():
            if value_template == "{prompt}":
                ...
                payload[key] = {k: v for k, v in parameters.items() if k not in ["api_key"]}
        ...
        try:
            ...
            for key in response_path.split('.'):
                ...
                elif isinstance(response_text, list) and key.isdigit() and int(key) < len(response_text):
