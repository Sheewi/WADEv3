...
class MemorySystem:
    ...
    def get_user(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                "SELECT id, name, preferences, created_at, last_active FROM users WHERE id = ?",
                (user_id,),
            )
            row = cursor.fetchone()
    ...
    def create_conversation(self, user_id: str, title: str = None) -> Optional[str]:
        ...
        try:
            ...
            now = datetime.datetime.now().isoformat()
            conversation_id = (
                f"conv_{now.replace(':', '').replace('.', '').replace('-', '')}"
            )
    ...
    def add_knowledge_item(
        self, user_id: str, content: str, source: str, tags: List[str] = None
    ) -> Optional[str]:
    ...
    def get_knowledge_items(
        self, user_id: str, tags: List[str] = None
    ) -> List[Dict[str, Any]]:
...
class AdaptiveLearning:
    ...
    def track_interaction(
        self, user_id: str, query: str, response: str, metadata: Dict[str, Any] = None
    ):
        ...
        try:
            ...
            current_topics = (
                self.memory.get_user_preference(user_id, "favorite_topics") or {}
            )
            for topic in topics:
            ...
            response_lengths = (
                self.memory.get_user_preference(user_id, "response_lengths") or []
            )
            response_lengths.append(len(response))
            if len(response_lengths) > 20:  # Keep only the last 20
                response_lengths = response_lengths[-20:]
            self.memory.set_user_preference(
                user_id, "response_lengths", response_lengths
            )

            # Track code preference
            has_code = "```" in response
            code_preferences = (
                self.memory.get_user_preference(user_id, "code_preferences") or []
            )
            ...
            self.memory.set_user_preference(
                user_id, "code_preferences", code_preferences
            )

            # Track active hours
            hour = datetime.datetime.now().hour
            active_hours = (
                self.memory.get_user_preference(user_id, "active_hours") or {}
            )
            ...
            if metadata and "tools_used" in metadata:
                tools_used = (
                    self.memory.get_user_preference(user_id, "tools_used") or {}
                )
        ...
        try:
            ...
            profile = {
                "favorite_topics": sorted(
                    favorite_topics.items(), key=lambda x: x[1], reverse=True
                )[:5],
                ...
                "code_preference": (
                    sum(code_preferences) / len(code_preferences)
                    if code_preferences
                    else 0.5
                ),
                "active_hours": sorted(
                    active_hours.items(), key=lambda x: int(x[1]), reverse=True
                )[:3],
                "favorite_tools": sorted(
                    tools_used.items(), key=lambda x: x[1], reverse=True
                )[:5],
            }
    ...
    def adapt_response(self, user_id: str, query: str, base_response: str) -> str:
        ...
        try:
            ...
            if (
                profile["code_preference"] > 0.7
                and "```" not in adapted_response
                and self._is_code_appropriate(query)
            ):
...
class WadeCore:
    ...
    async def process_query(
        ...
        model_name = self.model_router.route_query(
            query, self.memory.get_user_preferences(user_id)
        )
        ...
        # Add assistant message to memory
        self.memory.add_message(
            conversation_id, "assistant", adapted_response, {"model": model_name}
        )
        ...
        try:
            ...
            for prefix in [
                ...
                if prefix in query.lower():
                    description = query[
                        query.lower().find(prefix) + len(prefix) :
                    ].strip()
                    if (
                        description.startswith("that")
                        or description.startswith("which")
                        or description.startswith("to")
                    ):
                        description = description[description.find(" ") :].strip()
                    break

            # Create the agent
            agent_id = await self.agent_factory.create_agent_from_description(
                description
            )
    ...
    def _is_complex_task(self, query: str) -> bool:
        ...
        query_lower = query.lower()
        has_task_indicator = any(
            indicator in query_lower for indicator in task_indicators
        )
    ...
    async def _handle_complex_task(
        self, query: str, user_id: str, context: List[Dict[str, str]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not result["crew"]:
                ...
                response_data = await self.model_router.query_model(
            ...
            return {"response": response, "agent_ids": agent_ids, "success": True}
        except Exception as e:
...
async def main():
    ...
    result = await wade.process_query(
        "Find vulnerabilities in a WordPress site", user_id="test_user"
    )
