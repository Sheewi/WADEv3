#!/bin/bash
# Setup script for Wade with Crew AI integration on Kali Linux

set -e

echo "Setting up Wade with Crew AI - Multi-agent system for Kali Linux"
echo "=============================================================="

# Create directories
mkdir -p ~/.wade/{models,logs,config,tools,agents}

# Install dependencies
echo "Installing dependencies..."
if command -v apt-get &> /dev/null; then
    sudo apt-get update
    sudo apt-get install -y python3 python3-pip curl wget git nodejs npm xterm
elif command -v pacman &> /dev/null; then
    sudo pacman -Sy --noconfirm python python-pip curl wget git nodejs npm xterm
else
...
pip3 install --user flask flask-cors requests websockets crewai langchain langchain-community
...
# Copy the Crew AI integration script
echo "Setting up Crew AI integration..."
mkdir -p ~/.wade/core
cat > ~/.wade/core/crew_ai_integration.py << 'EOF'
#!/usr/bin/env python3
"""
...
chmod +x ~/.wade/core/crew_ai_integration.py

# Create the Flask API for the Crew AI system
echo "Creating API server..."
cat > ~/.wade/core/api_server.py << 'EOF'
#!/usr/bin/env python3
import os
import json
import asyncio
...
chmod +x ~/.wade/core/api_server.py

# Create a more advanced interface with Crew AI integration
echo "Creating advanced interface..."
mkdir -p ~/.wade/interface
cat > ~/.wade/interface/index.html << 'EOF'
<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
...
chmod +x ~/.wade/launch.sh

# Create desktop entry
echo "Creating desktop entry..."
mkdir -p ~/.local/share/applications
cat > ~/.local/share/applications/wade.desktop << 'EOF'
[Desktop Entry]
Name=Wade
...
sudo ln -sf ~/.wade/launch.sh /usr/local/bin/wade
