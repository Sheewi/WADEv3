...
class ModelPerformanceTracker:
    ...
    def record_usage(self, model_name: str, task_type: TaskType, 
                    response_time: float, success: bool, tokens_used: int):
        ...
        task_stats['avg_response_time'] = (
            (task_stats['avg_response_time'] * (task_stats['requests'] - 1) + response_time) 
            / task_stats['requests']
        )
    ...
    def get_model_score(self, model_name: str, task_type: TaskType) -> float:
        ...
        response_time_score = max(0, 1 - (task_stats['avg_response_time'] / 30))  # 30s baseline
...
class LLMRouter:
    ...
    def get_best_model(self, task_type: TaskType, agent_role: str = None) -> BaseChatModel:
        ...
        for name, config in specialized_models:
            ...
            performance_score = self.performance_tracker.get_model_score(name, task_type)
    ...
    def record_model_usage(self, model_name: str, task_type: TaskType, 
                          response_time: float, success: bool, tokens_used: int = 0):
    ...
    def get_model_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'model_configs': {name: {
                ...
                'specialties': [s.value for s in config.specialties] if config.specialties else [],
