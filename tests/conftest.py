...
def test_config():
    ...
    return {
        "server": {"host": "127.0.0.1", "port": 8080, "ssl": {"enabled": False}},
        "security": {
            "encryption_key": "test_key_12345678901234567890123456789012",
            "session_timeout": 3600,
            "max_login_attempts": 3,
        },
        "agents": {"max_concurrent": 2, "timeout": 60},
        "logging": {"level": "DEBUG", "file": "/tmp/wade_test.log"},
...
def mock_psutil():
    ...
    with patch("psutil.cpu_percent", return_value=50.0), patch(
        ...
        "psutil.boot_time", return_value=1000000
    ):
...
def mock_network():
    ...
    with patch("socket.socket") as mock_socket, patch("requests.get") as mock_requests:
...
def assert_metrics_valid(metrics_dict):
    ...
    for metric_name, value in metrics_dict.items():
        assert isinstance(
            metric_name, str
        ), f"Metric name must be string: {metric_name}"
        assert isinstance(
            value, (int, float)
        ), f"Metric value must be numeric: {metric_name}={value}"
        assert not metric_name.startswith(
            "_"
        ), f"Metric name should not start with underscore: {metric_name}"
