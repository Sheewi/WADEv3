...
from agents import (
    agent_manager,
)  # Agent manager is always imported here to support spawning any agent
...
class EliteFew:
    ...
    def __init__(self, wade_core_instance: Any):
        self.wade_core = (
            wade_core_instance  # Reference to the main WADE_OS_Core instance
        )
        self.log_and_respond(
            "EliteFew reasoning core active. Awaiting cerebral input.",
            component="ELITE_FEW",
        )
        ...
        self.ltm = (
            wade_core_instance.ltm
        )  # Shared instance from main, configured for crypto
        ...
        self.monk = self.agent_manager.spawn_agent(
            "Monk"
        )  # Behavioral analysis, critical for inference
        self.omen = self.agent_manager.spawn_agent(
            "Omen"
        )  # Risk assessment, pattern detection
        ...
        self.secret_manager = (
            wade_core_instance.secret_manager
        )  # Access to vault secrets

        # Elite Few operational state.
        self.current_context = {}  # Dynamic context built during interaction
        self.reasoning_depth = config.get(
            "reasoning_depth_default"
        )  # Rule 51: Adapts dynamically

    def log_and_respond(
        self,
        message: str,
        level: str = "INFO",
        component: str = "ELITE_FEW",
        response_to_user: bool = True,
        timestamp: Optional[float] = None,
    ):
        """Standardized logging and user interaction. All WADE output flows through here."""
        destabilized_message = destabilization.filter_hallucinations(
            message, config.get("hallucination_check_aggressiveness")
        )
        self.wade_core.log_event(
            destabilized_message, level=level, component=component, timestamp=timestamp
        )
        if response_to_user:
            ...
            if hasattr(self.wade_core, "gui_instance") and self.wade_core.gui_instance:
                self.wade_core.gui_instance.send_message_to_gui(
                    {
                        "level": level,
                        "component": component,
                        "message": destabilized_message,
    ...
    def process_user_command(self, command_string: str):
        ...
        self.log_and_respond(
            f"Processing command: '{command_string}'", component="INPUT_PROCESSOR"
        )

        # --- Pre-execution hooks: security and resource checks ---
        if not self.intrusion_detection.is_system_safe():
            self.log_and_respond(
                "System integrity warning: potential intrusion detected. Halting command processing.",
                level="ALERT",
            )
            self.wade_core.failsafe_trigger.activate_failsafe(
                "security_lockdown", "Potential Intrusion"
            )
            return

        if config.get("resource_governor_enabled") and not self.governor.can_execute(
            command_string
        ):
            self.log_and_respond(
                "Command aborted due to resource limitations or policy violation.",
                level="WARN",
            )
            return
        ...
        try:
            ...
            if config.get("challenge_logic_enabled") and not monk_logic_validation.get(
                "is_valid", True
            ):
                self.log_and_respond(
                    f"Accessing the user's implicit intent, your command currently contains logical inconsistencies or lacks precision. Refine your directive for optimal execution. Details: {monk_logic_validation.get('reason')}",
                    level="CHALLENGE",
            ...
            if not goal or confidence < config.get("goal_inference_threshold", 0.7):
                self.log_and_respond(
                    f"Command intent remains ambiguous (Confidence: {confidence:.2f}). To proceed, clarify your precise objective and expected outcome.",
                    level="CLARIFY",
                )
            ...
            self.log_and_respond(
                f"Inferred primary objective: '{goal}' with {confidence:.2f} confidence.",
                component="GOAL_INFERENCE",
            )
            self.stm.add_entry(
                "inferred_goal",
                {"goal": goal, "confidence": confidence, "command": command_string},
            )
            self.ltm.add_entry(
                "inferred_goals_history",
                {
            ...
            strategy = self._select_strategy(
                goal, self.current_context, self.stm, self.ltm
            )
            if not strategy:
                self.log_and_respond(
                    "No viable execution strategy identified for the current objective. Initiating deep system re-evaluation.",
                    level="CRITICAL",
                )
                feedback_loop.record_failure(
                    f"Strategy selection failed for goal: {goal}"
                )
                self.wade_core.evolution_engine.trigger_self_analysis(
                    f"Strategy selection impasse for '{goal}'",
                    "WADE_CORE/core_logic.py",
                    f"User Command: {command_string}",
                )
                return

            self.log_and_respond(
                f"Optimal strategy chosen: '{strategy.get('type')}' with an assessed risk score of {strategy.get('risk_score', 'UNKNOWN')}.",
            ...
            if not self._validate_strategy(strategy, goal):
                self.log_and_respond(
                    "Strategic validation failed. Environmental conditions or inherent risks preclude direct execution. Re-evaluating alternatives.",
                    level="CRITICAL",
                )
                feedback_loop.record_failure(f"Strategy validation failure for {goal}")
                self.wade_core.evolution_engine.trigger_self_analysis(
                    f"Strategy invalidation for '{goal}'",
                    "WADE_CORE/core_logic.py",
            ...
            action_plan = self._generate_action_plan(
                strategy, goal, self.current_context, self.stm, self.ltm
            )
            if not action_plan or not action_plan.get("steps"):
                self.log_and_respond(
                    "Failed to render an actionable plan. Core logic pathway obstructed. Mission aborted.",
                    level="CRITICAL",
                )
                feedback_loop.record_failure(
                    f"Action plan generation failed for {goal}"
                )
                self.wade_core.evolution_engine.trigger_self_analysis(
                    f"No actionable plan for '{goal}'",
                    "WADE_CORE/core_logic.py",
            ...
            self.log_and_respond(
                f"Execution plan drafted: '{action_plan['description']}'. Commencing tactical deployment.",
                component="PLANNING",
            )
            ...
            if final_result.get("status") == "error":
                self.log_and_respond(
                    f"Command '{command_string}' processing ended with errors: {final_result.get('message')}. Review logs.",
                    component="EXECUTION_SUMMARY",
                    level="ERROR",
                )
            else:
                self.log_and_respond(
                    f"Command '{command_string}' processing complete. Mission parameters satisfied.",
                    component="EXECUTION_SUMMARY",
                    level="INFO",
                )
            ...
            error_trace = (
                f"Exception during command processing: {e}\n{sys.exc_info()[2]}"
            )
            self.log_and_respond(
                f"[ERROR] Unhandled exception during command processing. Triggering urgent self-analysis. Details: {e}",
                level="ERROR",
            )
            feedback_loop.record_failure(
                f"Unhandled exception in process_user_command for '{command_string}': {e}"
            )
            self.wade_core.evolution_engine.trigger_self_analysis(
                f"Unhandled exception in core_logic: {e}",
                "WADE_CORE/core_logic.py",
                error_trace,
            )
    ...
    def _infer_goal(self, command_string: str) -> Tuple[str, float]:
        ...
        self.log_and_respond(
            "Initiating deep goal inference utilizing behavioral and contextual data...",
            level="DEBUG",
        )
        ...
        historical_goals = self.ltm.query_knowledge(
            "history",
            {"category": "inferred_goals_history", "user_id": "current"},
            limit=10,
        )

        # 2. Behavioral Pattern Matching (Monk Agent)
        monk_analysis = self.monk.execute_action(
            "pattern_match_behavior",
            {
                "command": command_string,
        ...
        if monk_analysis.get("status") == "success" and user_intent_pattern:
            self.log_and_respond(
                f"Monk identified critical behavioral pattern: '{user_intent_pattern}'. Adapting inference model.",
                level="INFO",
            )
            if user_intent_pattern == "recurring_recon":
                return "network_reconnaissance", 0.95
            if user_intent_pattern.startswith("prior_security_incident"):
                return "security_incident_response", 0.90
            if user_intent_pattern == "development_pattern":
                return "application_development", 0.85

        # 3. Direct keyword matching (fallback)
        if "scan network" in command_string.lower():
            return "network_reconnaissance", 0.85
        elif (
            "build app" in command_string.lower()
            or "create application" in command_string.lower()
        ):
            return "application_development", 0.80
        elif (
            "find exploit" in command_string.lower()
            or "vulnerability" in command_string.lower()
        ):
        ...
        self.log_and_respond(
            "Deep inference required for goal. No clear pattern match.", level="DEBUG"
        )
        return "", 0.3
    ...
    def _probe_user_for_clarification(self, original_command: str):
        ...
        self.log_and_respond(
            f"Your command '{original_command}' lacks clarity. State your goal with precision."
        )
        # CLI/GUI can present options or ask follow-up questions.

    def _select_strategy(
        self, goal: str, context: Dict[str, Any], stm, ltm
    ) -> Dict[str, Any]:
    ...
    def _validate_strategy(self, strategy: Dict[str, Any], goal: str) -> bool:
        ...
        for agent_name in strategy.get("agents", []):
            if not self.agent_manager.is_agent_available(agent_name):
                self.log_and_respond(
                    f"Strategy validation failed: Agent {agent_name} is not available.",
                    level="ERROR",
                )
                return False
        ...
        if strategy.get(
            "type"
        ) == "nmap_sweep_stealth" and not self.tool_manager.is_tool_available("nmap"):
            self.log_and_respond(
                "Strategy validation failed: Nmap tool is not available.", level="ERROR"
            )
            return False

        # Check for environmental constraints
        if (
            strategy.get("risk_score", 0) > 0.8
            and not self.sandbox_manager.is_sandbox_active()
        ):
            self.log_and_respond(
                "Strategy validation failed: High-risk operation requires active sandbox.",
                level="ERROR",
            )
            return False
    ...
    def _generate_action_plan(
        self, strategy: Dict[str, Any], goal: str, context: Dict[str, Any], stm, ltm
    ) -> Dict[str, Any]:
        ...
        if strategy.get("type") == "nmap_sweep_stealth":
            return {
                ...
                "steps": [
                    {
                        "agent": "Ghost",
                        "action": "establish_proxy_chain",
                        "params": {"type": "tor_chain"},
                    },
                    {
                        "agent": "Jarvis",
                        "action": "identify_target_range",
                        "params": {"context": "current_network"},
                    },
                    {
                        "agent": "Joker",
                        "action": "execute_nmap_scan",
                        "params": {
                            "target": "identified_range",
                            "scan_flags": "-sS -F --max-rate 50",
                            "proxy": "Ghost",
                        },
                    },
                ...
                "steps": [
                    {
                        ...
                        "params": {"context": "application_requirements"},
                    },
                    {
                        "agent": "Forge",
                        "action": "setup_project_structure",
                        "params": {"framework": "flask", "template": "basic"},
                    },
                    {
                        "agent": "Dexter",
                        "action": "implement_core_functionality",
                        "params": {"specs": "from_requirements"},
                    },
                    {
                        "agent": "Forge",
            ...
            return {
                ...
                "steps": [
                    {
                        "agent": "Ghost",
                        "action": "setup_secure_environment",
                        "params": {"anonymity_level": "high"},
                    },
                    {
                        "agent": "Joker",
                        "action": "scan_for_vulnerabilities",
                        "params": {"techniques": strategy["params"]["techniques"]},
                    },
                    {
                        "agent": "Dexter",
                        "action": "analyze_potential_exploits",
                        "params": {"findings": "from_scan"},
                    },
                    {
                        "agent": "Joker",
                        "action": "validate_vulnerabilities",
                        "params": {"exploits": "from_analysis"},
                    },
                ],
            }
                ...
                "steps": [
                    {
                        ...
                        "params": {"language": strategy["params"]["language"]},
                    },
                    {
                        "agent": "Oracle",
                        "action": "identify_patterns",
                        "params": {"code_structure": "from_analysis"},
                    },
                    {
                        "agent": "Dexter",
                        "action": "generate_recommendations",
                        "params": {"patterns": "from_identification"},
                    },
                ],
            }
