...
def setup_environment():
    ...
    timeline_path = os.path.join(
        wade_root, "WADE_RUNTIME", "logs", "event_timeline.json"
    )
...
class WADE_OS_Core:
    ...
    def start_gui(self):
        ...
        if not hasattr(self, "gui_instance") or not self.gui_instance:
            ...
            try:
                self.gui_instance = gui_overlay.initialize_gui(
                    self.elite_few, self.cli_handler
                )
                if not self.gui_instance:  # If GUI failed to initialize
                    self.log_event(
                        "GUI failed to initialize. Proceeding in CLI-only mode.",
                        level="ERROR",
                        component="GUI_INIT",
                    )
            except Exception as e:
                self.log_event(
                    f"Exception during GUI initialization: {e}. Proceeding in CLI-only mode.",
                    level="ERROR",
                    component="GUI_INIT",
                )
            ...
            self.log_event(
                "GUI thread already running.", level="DEBUG", component="GUI_INIT"
            )

    def run(self):
        """The main operational loop of WADE. Continuous interaction and execution."""
        self.log_event(
            "WADE_OS initiating main operational cycle. Your digital intelligence is online.",
            component="OPERATION_START",
        )
        ...
        while self.is_running:
            try:
                ...
                user_input = self.cli_handler.get_input(
                ...
                if user_input.lower().strip() == "exit":
                    ...
                    self.log_event(
                        "User initiated graceful shutdown. Protocols disengaging.",
                        component="SHUTDOWN_REQ",
                    )
                    break

                if (
                    user_input.lower().strip() == "emergency_wipe"
                ):  # Direct invocation of failsafe
                    self.elite_few.log_and_respond(
                        "EMERGENCY WIPE PROTOCOL ACTIVATED. SYSTEM NEUTRALIZATION INITIATED.",
                        level="CRITICAL",
                        response_to_user=True,
                    )
                    self.failsafe_trigger.activate_failsafe(
                        "perform_secure_wipe", "User Activated"
                    )
                    self.is_running = (
                        False  # System will self-destruct or reboot into neutral state
                    )
                    break
                ...
                self.log_event(
                    "KeyboardInterrupt detected. Initiating emergency shutdown sequence.",
                    level="CRITICAL",
                    component="SHUTDOWN_REQ",
                )
                self.is_running = False
                ...
                error_msg = (
                    f"Critical error in main loop: {e}. Traceback: {sys.exc_info()}"
                )
                self.elite_few.log_and_respond(
                    f"[ERROR] Unhandled exception during command processing. Triggering self-analysis. Details: {e}",
                    level="ERROR",
                    component="MAIN_LOOP_ERROR",
                )
                self.evolution_engine.trigger_self_analysis(
                ...
                time.sleep(
                    1
                )  # Prevent rapid error loop in case of recurring critical issue
    ...
    def shutdown(self):
        ...
        self.log_event(
            "WADE_OS executing shutdown sequence. Preserving state...",
            level="INFO",
            component="SHUTDOWN",
        )
        ...
        if self.intrusion_detection:
            self.intrusion_detection.stop_monitoring()  # Stop IDS
        config.save_config()  # Persist any runtime config changes.
        self.log_event(
            "WADE_OS shutdown complete. Master protocols disengaged.",
            level="INFO",
            component="SHUTDOWN",
        )
        sys.exit(0)
...
class FailsafeTrigger:
    ...
    def activate_failsafe(self, failsafe_type, reason):
        ...
        self.wade_core.log_event(
            f"FAILSAFE ACTIVATED: {failsafe_type} - Reason: {reason}",
            level="CRITICAL",
            component="FAILSAFE",
        )
...
if __name__ == "__main__":
    ...
    if os.geteuid() != 0:
        logging.warning(
            "WADE is not running as root. Functionality will be limited. For full environmental control, run with 'sudo' or via 'wade.service'."
        )
